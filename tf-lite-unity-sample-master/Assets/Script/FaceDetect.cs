...
namespace TensorFlowLite
{
    ...
    {
        public FaceDetectionSample fds = GameObject.Find("FaceDetection").GetComponent<FaceDetectionSample>();
        // 카메라로 분류하게 될 목록
        ...
        {
            if (fds.cur_ratio == FaceDetectionSample.Aspect_ratio.one_by_one)
            {
                ToTensor(inputTex, input0, 2);
            }
            else
            {
                ToTensor(inputTex, input0, 1);
            }
