...
namespace TensorFlowLite
{
    public abstract class BaseImagePredictor<T> : System.IDisposable where T : struct
    {
        ...
        protected TextureResizer.ResizeOptions resizeOptions2;
        ...
        public BaseImagePredictor(string modelPath, bool useGPU = true)
        {
            ...
            resizeOptions2 = new TextureResizer.ResizeOptions()
            {
        ...
        protected void ToTensor(Texture2D inputTex, float[,,] inputs)
        {
            RenderTexture tex = resizer.Resize(inputTex, resizeOptions);
            tex2tensor.ToTensor(tex, inputs);
        }
        protected void ToTensor(Texture inputTex, float[,,] inputs,int mode)
        {
            RenderTexture tex=null;
            if (mode == 1)
            {
                tex = resizer.Resize(inputTex, resizeOptions);
            }
            else if (mode == 2)
            {
                tex = resizer.Resize(inputTex, resizeOptions2);
            }
