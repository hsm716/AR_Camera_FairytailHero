...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    {
        #region Fields
        private GameObject Category_Scrollbar;
        ...
        private void Start()
        {
            Category_Scrollbar = GameObject.Find("Category_scrollbar");
            if (Validate())
            {
                Setup();
            }
        ...
        IEnumerator ScrollBar_ChangeValue(int panelNumber){
            yield return null;
            Category_Scrollbar.GetComponent<Scrollbar>().value = 0.11f * panelNumber;

        }
        public void GoToPanel(int panelNumber)
        {
            StartCoroutine(ScrollBar_ChangeValue(panelNumber));
